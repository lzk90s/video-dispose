...
private:
    ...
    void recogAttributesConv(rec::PersonAttributeGroupPO &in, algo::Attributes &out) {
    ...
    void recogAttributesConv(rec::VehicleAttributeGroup &in, algo::Attributes &out) {
    ...
    void buildAttr(const std::string &attrName, int32_t score, uint32_t attrType, algo::Attributes &out) {
        if (!attrName.empty()) {
...
private:
    ...
    std::unique_ptr<VideoProcService_Stub> stub_;
};
